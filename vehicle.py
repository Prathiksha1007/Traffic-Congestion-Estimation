...
class Vehicle:                                                                      #👇
    def __init__(self, screen, radius, width, speed, processed_vehicles, dti_info, vehicle_type="normal"):
...
#
        ...
        if self.vehicle_type == "emergency":
            self.speed *= 1.5          # move a bit faster
            self.color = (255, 255, 255)
        self.is_emergency = (self.vehicle_type == "emergency")
#
    ...
    def draw(self):
        if getattr(self, "is_emergency", False):
            # Flashing effect (alternates color)
            color = (255, 0, 0) if pygame.time.get_ticks() % 500 < 250 else (255, 255, 255)
            pygame.draw.circle(self.screen, color, (int(self.x), int(self.y)), self.radius + 3)
        else:
