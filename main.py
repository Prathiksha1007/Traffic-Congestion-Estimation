...
class Main:
    ...
    def vehicle_generator(self, stop_event, vehicle_list_lock):
        while not stop_event.is_set():
            ...
            '''
#
            # 5% chance of spawning an emergency vehicle
            vehicle_type = "emergency" if random.random() < 0.05 else "normal"

            vehicle = Vehicle(
                self.screen,
                self.vehicle_parameters["radius"],
                self.vehicle_parameters["width"],
                self.vehicle_parameters["speed"],
                self.vehicle_parameters["processed_vehicles"],
                self.vehicle_parameters["dti_info"],
                vehicle_type  # pass new type
            )

            vehicle.generate_vehicle(
                self.vehicle_spawn_coords,
                self.vehicle_parameters["incoming_direction"],
                self.colors["vehicle_direction"],
...
#
    ...
    def run(self, generation=None, training=False, end_count=None, action_list=None):
        ...
        vehicle_gen_thread = threading.Thread(target=self.vehicle_generator,
                                              args=(stop_event, self.vehicle_list_lock))
        try:
...
#
                        ...
                        for direction, lane_info in self.vehicle_parameters["dti_info"].items():
                            # Check if any emergency vehicle exists in this lane
                            if self.detect_emergency_vehicle(lane_info):  # We'll define this function below
                                print(f"🚨 Emergency vehicle detected in {direction}! Giving priority.")
                                current_action = self.direction_to_action_index(direction)  # Map to action index (e.g. 0=N,1=E,...)
                                break

                        # Apply the final (possibly overridden) action
                        self.apply_action(current_action, traffic_lights)
                        '''
                        # 🚨 Emergency Vehicle Priority Override
                        emergency_detected = False
                        # DEBUG: count active emergency vehicles (thread-safe)
                        with self.vehicle_list_lock:
                            emergency_count = sum(1 for v in self.vehicle_list if getattr(v, "is_emergency", False))

                        for direction in self.traffic_light_parameters["directions"]:
                            if self.detect_emergency_vehicle(direction):
                                print(f" Emergency vehicle detected in {direction}! Giving priority.")
                                current_action = self.direction_to_action_index(direction)
                                self.apply_action(current_action, traffic_lights)
                                emergency_detected = True
                                break  # give priority immediately

                        # If no emergency vehicle, proceed normally
...
#
                        ...
                        if not emergency_detected:
                            ...
                            # Skip updating SARSA for forced emergency override to avoid corrupting learning.
                            # You can still set last_action_time if desired.
                            next_action = None
#
...
#
    def detect_emergency_vehicle(self, direction):
        """Check if any emergency vehicle is present in the given direction."""
        with self.vehicle_list_lock:
            for v in self.vehicle_list:
                if v.direction == direction and getattr(v, "is_emergency", False):
                    return True
        return False
#
    '''
#
    def detect_emergency_vehicle(self, lane_info):
        """Check if an emergency vehicle is present in the lane."""
        for vid, vinfo in lane_info.items():
            # Assuming each vehicle dict has a type or color property
            if vinfo.get("type") == "emergency" or vinfo.get("is_emergency", False):
                return True
        return False
#
    '''
#
    def direction_to_action_index(self, direction):
        """Map direction (string) to the corresponding action index."""
